...
export default function GEPSearch({setGepSearchTabData, gepSearchData}: {setGepSearchTabData: (key: keyof GEPData, value: any) => void, gepSearchData: GEPData}) {
    ...
    }, [gepSearchData.selectedTerm, gepSearchData.searchSubject, setGepSearchTabData]);
    // const getItemSize = (index: number) => {
    //   const course = filteredCourses[index];
    //   const header_plus_padding_const = 120;
    ...
    //   const hasSections = courseData && courseData[key]?.sections?.length > 0; 
    //   if (hasSections) {
    //     return header_plus_padding_const + courseData[key].sections.length * 52;
    //   }
    //   return header_plus_padding_const;
    // };
    function RowVirualizer({parentRef}: {parentRef: React.RefObject<HTMLDivElement | null>}) {
      ...
      const items = rowVirtualizer.getVirtualItems();
        ...
        >
          {items.map((virtualRow) => (
              ...
              >
                  <ListItem alignItems="flex-start" key={filteredCourses[virtualRow.index].course_abr} sx={{ pl: 4 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                      <ListItemText
                        primary={`${filteredCourses[virtualRow.index].course_descrip} ${filteredCourses[virtualRow.index].course_abr} ${parseInt(filteredCourses[virtualRow.index].catalog_num)}`}
                        secondary={`${filteredCourses[virtualRow.index].course_abr} ${filteredCourses[virtualRow.index].catalog_num}`}
                      />
                      {(gepSearchData.courseData as Record<string, MergedCourseData>)[`${filteredCourses[virtualRow.index].course_abr} ${filteredCourses[virtualRow.index].catalog_num}`]?.sections?.length > 0 ? (
                        <Box sx={{ 
                          height: '100%',
                          width: '100%',
                          display: 'flex'
                          ...
                          <MemoizedDataGrid
                            sections={(gepSearchData.courseData as Record<string, MergedCourseData>)[`${filteredCourses[virtualRow.index].course_abr} ${filteredCourses[virtualRow.index].catalog_num}`]?.sections || []}
    ...
    return (
      <StyledDialogContent ref={parentRef} sx={{ overflow: 'auto' }}>
        <Box sx={{ width: '100%', p: 2 }}>
          <List>
            ...
            <FormControlLabel
            control={<Checkbox checked={gepSearchData.hideNoSections} onChange={handleHideNoSectionsChange} />}
        ...
        <RowVirualizer key={`course-list-${gepSearchData.hideNoSections ? 'filtered' : 'all'}`} parentRef={parentRef} />
      </StyledDialogContent>
