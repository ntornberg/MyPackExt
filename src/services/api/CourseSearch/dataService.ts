...
export async function batchFetchCoursesData(
  ...
  if (sectionsToFetch.length > 0) {
    ...
    const url = `${SUPABASE_URL}/functions/v1/quick-service`;
    const profDataResponseApi = await fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `Bearer ${SUPABASE_ANON_KEY}`
    ...
    if (!profDataResponseApi.ok) {
      throw new Error(`API error: ${profDataResponseApi.status} ${profDataResponseApi.statusText}`);
    }
    const jsonResponse = await profDataResponseApi.json();
    const responseData = jsonResponse.data as MatchedRateMyProf[];
    gradeProfData.profs = responseData;

    const gradientQueries = sectionsToFetch.reduce<Record<string, Record<string, string[]>>>((acc: Record<string, Record<string, string[]>>, course) => {
      const { course_title, instructor_name } = course;
      if (!acc[course_title.split(" ")[0]]) {
        acc[course_title.split(" ")[0]] = {};
      }
      if (!acc[course_title.split(" ")[0]][course_title.split(" ")[1]]) {
        acc[course_title.split(" ")[0]][course_title.split(" ")[1]] = [];
      }

      acc[course_title.split(" ")[0]][course_title.split(" ")[1]].push(instructor_name);
      return acc;

    }, {});


    function sendGradientData(subject: string, course: string, instructors: string[]): Promise<{ success: boolean, data: RootObject }> {
      return new Promise((resolve, _) => {
        chrome.runtime.sendMessage({
          type: "get_gradient_data", data: {
            subject: subject,
    ...
    for (const [subject, course_plus_ins] of Object.entries(gradientQueries)) {
      for (const [cat_num, instructors] of Object.entries(course_plus_ins)) {

        const gradientResponse = await sendGradientData(subject, cat_num, instructors);
        const gradeApiResponse = parseGradientResponse(subject, cat_num, gradientResponse.data) as GradeData[];
        gradeData.push(...gradeApiResponse);
      }
    }
    gradeProfData.courses = gradeData;
  }
