...
async function openDatabase(): Promise<IDBDatabase> {
  AppLogger.info(
    `[CACHE DB OPEN] Opening IndexedDB database: ${DB_NAME}, version: ${DB_VERSION}`,
  );
    ...
    const attemptOpen = () => {
      try {
        ...
        request.onerror = (_) => {
          ...
          AppLogger.error(
            `[CACHE DB ERROR] Failed to open IndexedDB: ${error.message}`,
          );

          if (retryCount < maxRetries) {
            retryCount++;
            AppLogger.info(
              `[CACHE DB RETRY] Retrying database open (${retryCount}/${maxRetries})...`,
            );
        ...
        request.onsuccess = (_) => {
          ...
          db.onerror = (event) => {
            AppLogger.error(
              `[CACHE DB ERROR] Database error: ${(event.target as any).errorCode}`,
            );
          };
        ...
        request.onupgradeneeded = (_) => {
          AppLogger.info(
            `[CACHE DB UPGRADE] Database upgrade needed for ${DB_NAME}`,
          );
          const db = request.result;
          ...
          if (!db.objectStoreNames.contains(STORE_NAME)) {
            AppLogger.info(
              `[CACHE DB CREATE] Creating object store: ${STORE_NAME}`,
            );
        ...
        AppLogger.error(
          `[CACHE DB ERROR] Exception opening database: ${error}`,
        );
        reject(error);
      }
    };
 ...
 */
export async function getCacheCategory(
  cacheCategory: string,
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        getAllKeysRequest.onerror = () => {
          AppLogger.error(
            `[CACHE ERROR] Failed to get keys from IndexedDB:`,
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        request.onsuccess = () => {
          if (request.result) {
            ...
            if (entry.expiresAt && now > entry.expiresAt) {
              ...
              try {
                if (db) {
                  ...
                  deleteTransaction.oncomplete = () => {
                    AppLogger.info(
                      `[CACHE DB] Deleted expired entry: ${cacheCategory}:${hash}...`,
                    );
                  };
                }
              } catch (deleteError) {
                AppLogger.error(
                  `[CACHE DB ERROR] Failed to delete expired entry: ${deleteError}`,
                );
              }
              ...
              AppLogger.info(
                `[CACHE DB HIT] Found valid entry in IndexedDB: ${cacheCategory}:${hash}...`,
              );
              resolve(entry);
            }
          } else {
            AppLogger.info(
              `[CACHE DB MISS] No entry found in IndexedDB: ${cacheCategory}:${hash}...`,
            );
            resolve(null);
          }
        };
        ...
        request.onerror = () => {
          AppLogger.error(
            `[CACHE DB ERROR] Failed to get from IndexedDB: ${request.error?.message || "Unknown error"}`,
          );
 ...
 */
export async function getGenericCache(
  cacheCategory: string,
  hash: string,
): Promise<CacheEntry | null> {
  try {
    if (isChromeStorageAvailable()) {
      try {
        ...
        AppLogger.error(
          `[CACHE ERROR] Chrome storage get failed: ${chromeError}`,
          chromeError,
        );
      }
    } else {
      AppLogger.info(
        `[CACHE INFO] Chrome storage not available, trying IndexedDB`,
      );
    }
    try {
      ...
      AppLogger.error(
        `[CACHE ERROR] IndexedDB get failed: ${indexedDBError}`,
        indexedDBError,
      );
      return null;
    }
    ...
    AppLogger.error(
      `[CACHE ERROR] getGenericCache: Category: ${cacheCategory}, Key: ${hash}, Error: ${error}`,
      error,
    );
    return null;
  }
 ...
 * @returns {Promise<void>}
 */
export async function setGenericCache(
  cacheCategory: string,
  cacheEntries: Record<string, any>,
  cacheExpirationOverride?: number,
): Promise<void> {
  try {
    const dataAsString =
      typeof cacheEntries === "string"
        ? cacheEntries
    ...
    if (dataSize > SIZE_THRESHOLD || !isChromeStorageAvailable()) {
      try {
        ...
        AppLogger.error(
          `[CACHE ERROR] IndexedDB storage failed: ${indexedDBError}`,
          indexedDBError,
        );
    ...
    try {
      const currentCache: Record<string, CacheEntry> =
      ...
      AppLogger.info(
        `[CACHE SUCCESS] Successfully stored in Chrome storage: ${cacheCategory}:${Object.keys(cacheEntryList)}...`,
      );
    } catch (chromeStorageError) {
      AppLogger.error(
        `[CACHE ERROR] Chrome storage failed: ${chromeStorageError}`,
        chromeStorageError,
      );
      ...
      try {
        ...
        AppLogger.error(
          `[CACHE ERROR] Fallback to IndexedDB also failed: ${fallbackError}`,
          fallbackError,
        );
        throw fallbackError;
      }
    ...
    AppLogger.error(
      `[CACHE ERROR] setGenericCache: Category: ${cacheCategory}, Error: ${error}`,
      error,
    );
    throw error;
  }
...
export async function clearGenericCache(cacheCategory: string): Promise<void> {
  try {
    ...
    AppLogger.error(
      `[CACHE ERROR] clearGenericCache: Category: ${cacheCategory}`,
      error,
    );
  }
}
